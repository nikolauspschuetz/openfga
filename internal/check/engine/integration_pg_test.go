...
// These integration tests drive the engine end to end against a real PostgreSQL database in a
...
//
// The engine's other tests pin plan-tree shape and golden SQL against a fake builder; this suite
...
//
// Gated behind the `docker` build tag so the engine package stays Docker-free in the
// `make test-unit` / `make test` lanes; run with `go test -tags=docker ./internal/check/engine/...`.
package engine
...
// pg adapter the engine emits queries against. Both point at the same container, which
...
// container's database never collide) and returns the engine's decision.
func (e *pgEnv) run(t *testing.T, tc engineCase) bool {
	t.Helper()
...
func (e *pgEnv) runCases(t *testing.T, cases []engineCase) {
	t.Helper()
...
// TestEngineIntegrationPostgres_ConditionFree covers the HAVING path, where Postgres folds the
// whole set algebra and a returned row means granted.
func TestEngineIntegrationPostgres_ConditionFree(t *testing.T) {
	setupPgEnv(t).runCases(t, conditionFreeCases(t))
}

// TestEngineIntegrationPostgres_NestedConditionFree drives the complex nested tree end to end,
// the integration analogue of TestPlanSQL_NestedSetOperationsHaving.
func TestEngineIntegrationPostgres_NestedConditionFree(t *testing.T) {
	setupPgEnv(t).runCases(t, nestedConditionFreeCases(t))
}

// TestEngineIntegrationPostgres_NestedConditioned drives the conditioned nested tree through the
// gather path, the integration analogue of TestPlanSQL_NestedSetOperationsWithConditionsGather.
func TestEngineIntegrationPostgres_NestedConditioned(t *testing.T) {
	setupPgEnv(t).runCases(t, nestedConditionedCases(t))
}

// TestEngineIntegrationPostgres_WeightTwo drives weight-2 resolution paths — a single
// tuple-to-userset or userset hop — end to end against Postgres.
func TestEngineIntegrationPostgres_WeightTwo(t *testing.T) {
	setupPgEnv(t).runCases(t, weightTwoCases(t))
}

// TestEngineIntegrationPostgres_WeightTwoComplexBothLevels drives the complex combination of set
// operations across hops and within a hop end to end against Postgres.
func TestEngineIntegrationPostgres_WeightTwoComplexBothLevels(t *testing.T) {
	setupPgEnv(t).runCases(t, weightTwoComplexBothLevelsCases(t))
}

// TestEngineIntegrationPostgres_WeightTwoCondLeft drives the LEFT-side conditioned model: each
// hop-1 edge carries a condition, the hop-2 relations do not.
func TestEngineIntegrationPostgres_WeightTwoCondLeft(t *testing.T) {
	setupPgEnv(t).runCases(t, weightTwoCondLeftCases(t))
}

// TestEngineIntegrationPostgres_WeightTwoCondRight drives the RIGHT-side conditioned model: the
// hop-2 relations carry conditions, the hop-1 edges do not.
func TestEngineIntegrationPostgres_WeightTwoCondRight(t *testing.T) {
	setupPgEnv(t).runCases(t, weightTwoCondRightCases(t))
}

// TestEngineIntegrationPostgres_WeightTwoCondBoth drives the BOTH-sides conditioned model: every
// traversal carries a condition on its hop-1 edge and its hop-2 relation.
func TestEngineIntegrationPostgres_WeightTwoCondBoth(t *testing.T) {
	setupPgEnv(t).runCases(t, weightTwoCondBothCases(t))
}

// TestEngineIntegrationPostgres_WeightTwoComplexMixed drives the combined fixture: TTU and userset
// hops mixed across union/intersection/exclusion, with conditions on the left, the right, and both.
func TestEngineIntegrationPostgres_WeightTwoComplexMixed(t *testing.T) {
	setupPgEnv(t).runCases(t, complexWeightTwoMixedCases(t))
}

// TestEngineIntegrationPostgres_MergedExclusion drives the merged-region exclusion model end to
...
func TestEngineIntegrationPostgres_MergedExclusion(t *testing.T) {
	setupPgEnv(t).runCases(t, mergedExclusionCases(t))
}

// TestEngineIntegrationPostgres_Conditioned covers the gather path on weight-1 relations: the
// plan mentions an ABAC condition, so Postgres only scans candidate tuples and the engine folds
// the set algebra in process after evaluating CEL.
func TestEngineIntegrationPostgres_Conditioned(t *testing.T) {
	setupPgEnv(t).runCases(t, conditionedCases(t))
}

// TestEngineIntegrationPostgres_StaleConditionOnConditionFreeRelation guards a data-integrity
...
func TestEngineIntegrationPostgres_StaleConditionOnConditionFreeRelation(t *testing.T) {
	setupPgEnv(t).runCases(t, staleConditionCases(t))
}

// TestEngineIntegrationPostgres_Cycle drives models with relationship cycles end to end against
...
func TestEngineIntegrationPostgres_Cycle(t *testing.T) {
	e := setupPgEnv(t)
